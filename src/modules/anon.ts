...
function handleResponce(msg) {
  ...
  if (
    anonMessageServerChoice[senderID].anonChannelSelect &&
    !isNaN(msg.content) &&
    Number(msg.content) <=
      anonMessageServerChoice[msg.author.id].anonChannelSelect.guildChannelIDs
        .length +
        1 &&
    Number.isInteger(Number(msg.content))
  ) {
    const chosenChannel =
      anonMessageServerChoice[senderID].anonChannelSelect.guildChannelIDs[
    ...
    fs.writeFile(
      path.join(__dirname, "..", "..", "..", "src", "data.json"),
      JSON.stringify(anonMessageServerChoice),
      (err) => {
        if (err) throw err;
      }
    );
  ...
  if (
    anonMessageServerChoice[senderID].anonServerSelect &&
    !isNaN(msg.content) &&
    Number(msg.content) <=
      anonMessageServerChoice[msg.author.id].anonServerSelect.commonGuildIDs
        .length +
        1 &&
    Number.isInteger(Number(msg.content))
  ) {
    const chosenGuild =
      anonMessageServerChoice[senderID].anonServerSelect.commonGuildIDs[
        Number(msg.content) - 1
      ]; //add  greater than zero
    const message = anonMessageServerChoice[senderID].anonServerSelect.message;
    const guildChannelNames = [];
    bot.guilds.cache
      .find((guild) => guild.id === chosenGuild)
    ...
    const guildChannelNamesString = guildChannelNames
      ...
      .join("\n");
    anonMessageServerChoice[senderID] = {
      anonChannelSelect: {
    ...
    if (msg.content.startsWith("!anon ")) {
      ...
      if (messageArray[1].startsWith("!")) {
        ...
        if (commonGuilds.length === 1) {
          const guildChannelNames = [];
          bot.guilds.cache
            .find((guild) => guild.id === commonGuilds[0].id)
            .channels.cache.forEach((channel) => {
              if (channel.type === "text") {
                guildChannelNames.push(channel.name);
              }
          ...
          msg.reply(
            `The discord server "${commonGuilds[0].name}" has ${guildChannelIDs.length} text channels.\nPlease type the number of the channel you would like to send your anonymous message to:\n${guildChannelNamesString}`
          );
        }
        if (commonGuilds.length > 1) {
